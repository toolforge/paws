resource "openstack_containerinfra_cluster_v1" "k8s_123_10" {
  name                = "paws${var.name[var.datacenter]}-123-10"
  cluster_template_id = resource.openstack_containerinfra_clustertemplate_v1.template_123_10.id
  master_count        = 1
  node_count          = var.workers[var.datacenter]
}

resource "local_file" "kube_config" {
  content  = resource.openstack_containerinfra_cluster_v1.k8s_123_10.kubeconfig.raw_config
  filename = "kube.config"
}

resource "openstack_containerinfra_clustertemplate_v1" "template_123_10" {
  name                  = "paws${var.name[var.datacenter]}-123-10"
  coe                   = "kubernetes"
