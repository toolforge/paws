resource "openstack_containerinfra_cluster_v1" "k8s_128" {
  name                = "paws${var.name[var.datacenter]}-128"
  cluster_template_id = resource.openstack_containerinfra_clustertemplate_v1.template_128.id
  master_count        = 1
  node_count          = var.workers[var.datacenter]
}


resource "openstack_containerinfra_clustertemplate_v1" "template_128" {
  name                  = "paws${var.name[var.datacenter]}-128"
  coe                   = "kubernetes"
