resource "openstack_containerinfra_cluster_v1" "k8s_124_1" {
  name                = "paws${var.name[var.datacenter]}-124-1"
  cluster_template_id = resource.openstack_containerinfra_clustertemplate_v1.template_124_1.id
  master_count        = 1
  node_count          = var.workers[var.datacenter]
}

resource "local_file" "kube_config" {
  content  = resource.openstack_containerinfra_cluster_v1.k8s_124_1.kubeconfig.raw_config
  filename = "kube.config"
}

resource "openstack_containerinfra_clustertemplate_v1" "template_124_1" {
  name                  = "paws${var.name[var.datacenter]}-124-1"
  coe                   = "kubernetes"
  ...
  labels = {
    kube_tag               = "v1.24.17-rancher1-linux-amd64"
